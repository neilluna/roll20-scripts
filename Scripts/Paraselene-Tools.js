...
const ParaseleneTools = (() => {
  ...
  const getTokenInfo = (msg) => {
    ...
    sendChat(
      speakAs,
      `!${scriptName}-Get-Token-Info-API --speakAs ${speakAs} ${playerId} ${tokenId}`,
    );
  };
  ...
  const getTokenInfoAPI = (msg) => {
    ...
    const table = new Table()
      ...
      .add(
        new Row().add(new Header("id", cellStyle)).add(new Cell(id, cellStyle)),
      )
      .add(
        new Row()
  ...
  const deleteTokenAPI = (msg) => {
    ...
    pc.sendChatNoArchive(
      speakAs,
      `/w "${playerName}" <br/>Token "${name}" (${id}) deleted.`,
    );
  };
  ...
  const listTokensAPI = (msg) => {
    ...
    const pageTokensTable = new Table()
      ...
      .add(
        new Row()
          ...
          .add(
            new Header("left,&nbsp;top,&nbsp;width,&nbsp;height", cellStyle),
          ),
      );
      ...
      pageTokensTable.add(
        new Row()
          .add(
            new Cell(
              new Link(
                `!${scriptName}-Get-Token-Info-API --speakAs ${speakAs} ${playerId} ${id}`,
                name,
              ).render(),
            ),
          )
    ...
    pc.sendChatNoArchive(
      speakAs,
      `/w "${playerName}" <br/>${pageTokensTable.render()}`,
    );
  };
  ...
  const pingCharacter = (msg) => {
    ...
    if (playerIsGM(playerId)) {
      const message =
        "GMs cannot use Ping-Character. Use Get-Token-Info instead.";
    ...
    const characters = findObjs({
      ...
      const isControlledBy =
        controlledBy.includes(playerId) || controlledBy.includes("all");
      const isInPlayerJournals =
        inplayerjournals.includes(playerId) || inplayerjournals.includes("all");
    ...
    if (tokens.length == 0) {
      pc.sendChatNoArchive(
        speakAs,
        `/w "${playerName}" <br/>None of your tokens are on this map.`,
      );
      return;
    }
    ...
    const cellStyle = "padding-left: 5px; padding-right: 5px;";
    const pageTokensTable = new Table().add(
      new Row()
        .add(new Header("Token", cellStyle))
        .add(new Header("left,&nbsp;top,&nbsp;width,&nbsp;height", cellStyle)),
    );
  ...
  const pingTokenAPI = (msg) => {
    ...
    pc.sendChatNoArchive(
      speakAs,
      `/w "${playerName}" <br/>Pinging token ${name} ...`,
    );
  ...
  const listPullTokensAPI = (msg) => {
    ...
    if (pageTokens.length == 0) {
      pc.sendChatNoArchive(
        speakAs,
        `/w "${playerName}" <br/>There are no tokens on this map.`,
      );
      return;
    }
  ...
  const whisperTokenNotSelected = (msg) => {
    ...
    const speakAs = pc.extractCommandLineOption(args, "--speakAs", commandName);
    const isCharacter =
      pc.extractCommandLineOption(args, "--isCharacter", "false") == "true";
    const playerId = msg.playerid;
    const player = getObj("player", playerId);
    const playerName = player.get("displayname");
