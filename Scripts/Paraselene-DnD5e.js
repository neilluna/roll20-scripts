...
const ParaseleneDnD5e = (() => {
  ...
  const addAoePattern = (msg) => {
    ...
    while (index < args.length) {
      ...
      if (!Object.prototype.hasOwnProperty.call(spells, arg)) {
        pc.sendChatNoArchive(
          speakAs,
          `/w "${playerName}" <br/>"${arg}" is not recognized.`,
        );
      }
      spells[arg] = true;
    }
    ...
    const menu = [
      `&{template:traits}{{name=Add AOE Pattern}} {{description=${token.get("name")}`,
    ];
    if (spells.AcidSplash) {
      menu.push("[Acid Splash](~ParaseleneDnD5e|AddAOEAcidSplash)");
    }
    if (spells.AuraOfProtection) {
      menu.push(
    ...
    if (spells.Grease) {
      menu.push("[Grease](~ParaseleneDnD5e|AddAOEGrease)");
    }
    ...
    if (spells.LightningBolt) {
      menu.push("[Lightning Bolt](~ParaseleneDnD5e|AddAOELightningBolt)");
    }
    ...
    if (spells.PassWithoutTrace) {
      menu.push(
        "[Pass Without Trace](~ParaseleneDnD5e|AddAOEPassWithoutTrace)",
      );
    }
    if (spells.Shatter) {
  ...
  const castSpell = (msg) => {
    ...
    const menu = [
      `&{template:traits}{{name=Cast Spell}} {{description=${token.get("name")}`,
    ];
    if (spells.ChaosBolt) {
