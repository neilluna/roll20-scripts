...
const ParaseleneCommon = (() => {
  ...
  const getPlayerPageId = (playerId) => {
    ...
    if (
      playerPages != false &&
      Object.prototype.hasOwnProperty.call(playerPages, playerId)
    ) {
  ...
  const whisperTokenDoesNotExist = (speakAs, playerName) => {
    sendChatNoArchive(
      speakAs,
      `/w "${playerName}" <br/>That token is no longer in the game.`,
    );
  };
  ...
  const whisperTokenNotOnPage = (speakAs, playerName) => {
    sendChatNoArchive(
      speakAs,
      `/w "${playerName}" <br/>That token is no longer on this map.`,
    );
  };

  // Send a chat message to a player that a token is not selected.
  const whisperTokenNotSelected = (
    speakAs,
    playerName,
    isCharacter = false,
  ) => {
    if (isCharacter) {
      sendChatNoArchive(
        speakAs,
        `/w "${playerName}" <br/>No character token is selected.`,
      );
    } else {
      sendChatNoArchive(
        speakAs,
        `/w "${playerName}" <br/>No token is selected.`,
      );
    }
  };
